...
\begin{frame}{9.4 Modèle leads \& lags (forme standard)}
\begin{block}{Régression event study}
\[
Y_{it}=\alpha_i+\lambda_t+\sum_{\tau=-q}^{-1}\gamma_{\tau}\,D_{i,t+\tau}
+\sum_{\tau=0}^{m}\delta_{\tau}\,D_{i,t+\tau}+X_{it}\beta+\varepsilon_{it}
\]
\end{block}
\begin{itemize}
  \item $\gamma_\tau$ : \textbf{leads} (placebo avant traitement).
  \item $\delta_\tau$ : \textbf{lags} (effets après traitement).
  \item Lecture : si les leads $\approx 0$ avant, c’est rassurant.
\end{itemize}
% \note{Expliquer le “temps 0 = année de traitement”.}
\end{frame}

\begin{frame}{Event Study : dynamique des effets}

\centering
\includegraphics[height=0.75\textheight, keepaspectratio]{94.png}
...
\begin{block}{Emplacement figures (très utile)}
\begin{itemize}
  \item 1 figure ``first stage'' (éligibilité/couverture).
  \item 1 figure outcome (mortalité).
\end{itemize}
\end{block}
